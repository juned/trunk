...
if ( $params->{cdr} ) { # PROCESS CDRs.
      ...
      {      
          ...
          while ( $parentid > 0 ) {
                ...
                my $cdr_string = "".$gbl_astpp_db->quote($data->{variables}->{uuid}).",".$accountid.",".$gbl_astpp_db->quote(uri_unescape($data->{variables}->{caller_id})).",".$gbl_astpp_db->quote($data->{variables}->{effective_destination_number}).",".$actual_duration.",".$gbl_astpp_db->quote($data->{variables}->{hangup_cause}).",".$gbl_astpp_db->quote(uri_unescape($data->{variables}->{callstart})).",".$debit.",".$cost.",".$origination_rate->{$accountid}->{RATEGROUP}.",".$data->{variables}->{package_id}.",".$gbl_astpp_db->quote($origination_rate->{$accountid}->{CODE} or '').",".$gbl_astpp_db->quote($origination_rate->{$accountid}->{DESTINATION} or '').",".($origination_rate->{$accountid}->{COST} or '0').",".$parentid.",".$gbl_astpp_db->quote($origination_rate->{$parentid}->{CODE} or '').",".$gbl_astpp_db->quote($origination_rate->{$parentid}->{DESTINATION} or '').",".($origination_rate->{$parentid}->{COST} or '0').",".$gbl_astpp_db->quote($data->{variables}->{call_direction}).",".$gbl_astpp_db->quote($data->{variables}->{calltype})."";
